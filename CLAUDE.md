...
- `outputs/`: kết quả chạy (kể cả `outputs.zip` tải từ Kaggle), chỉ ở máy; repo chỉ giữ `outputs/.gitkeep`. Số đo
  hiệu năng (GPU, renderer OpenGL, thời gian từng bước) nằm trong `run_info.json` của `outputs.zip`: đọc file đó,
  đừng dựa vào log. Muốn thêm số đo thì ghi vào `RUN_INFO` trong `build_notebook.py`.
